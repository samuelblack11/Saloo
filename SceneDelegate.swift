...
class SceneDelegate: UIResponder, UIWindowSceneDelegate, ObservableObject {
    ...
    var gotRecord = false
    @StateObject var appDelFromSceneDel = AppDelegate()
    /**
    ...
    func parseRecord(record: CKRecord?) {
        ...
        DispatchQueue.main.async() {
            ...
            print(self.gotRecord)
            print(self.coreCard?.cardName)
            print(self.coreCard?.spotPreviewURL!)
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if let windowScene = scene as? UIWindowScene {
            Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { timer in
                if self.gotRecord {
                    let contentView = EnlargeECardView(chosenCard: self.coreCard!, share: self.acceptedShare, cardsForDisplay: self.loadCoreCards(), whichBoxVal: self.whichBoxForCKAccept!)
